...
class AssetQR3Screen extends React.Component {
    ...
    onSuccess(e) {
        ...
        for (let i = 0; i < this.props.returned_stocks.length; i++) {
            if (JSON.parse(this.props.returned_stocks[i].value).stock_serial == e.data) {
                count++
            }
        }
        if (count > 0) {
            ...
            fetch('https://api.motiveone.com.au/motiveOne/mobileApi/actions/getDispatchedStock', {
                method: 'POST',
                headers: {
                ...
                .then((responseJson) => {
                    if (responseJson.success == 1) {
                        let stock = { stock_serial: e.data, stock_description: responseJson.data.stock_description, customer_name: responseJson.data.customer_name }
    ...
    onSearch() {
        this.props.navigation.navigate('SearchSerial_Return', {
            route: 'Return'
        })
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(AssetQR3Screen)