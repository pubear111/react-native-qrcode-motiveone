...
class SearchSerial_ReturnScreen extends React.Component {
    ...
    onSelect(item) {
        ...
        for (let i = 0; i < this.props.returned_stocks.length; i++) {
            if (JSON.parse(this.props.returned_stocks[i].value).stock_serial == item.stock_serial) {
                count++
            }
        }
        if (count > 0) {
            this.refs.toast.show('This stock was scanned already', 2000);
            setTimeout(() => { this.props.navigation.navigate('Return') }, 2000);
        } else {
            let stock = { stock_serial: item.stock_serial, stock_description: item.stock_description, customer_name: item.customer_name }
            this.props.actions.returnedStock(JSON.stringify(stock))
            this.props.navigation.replace('Return')
        }
    }

    onHome() {
        if (this.props.navigation.state.params.route == 'Home') {
...
export default connect(mapStateToProps, mapDispatchToProps)(SearchSerial_ReturnScreen)